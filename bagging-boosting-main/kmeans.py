...
train = pd.DataFrame(training_data_2c, columns=[
                     'height', 'diameter', 'weight', 'hue', 'material'])
y_train = train["material"]
x_train = train[['height', 'diameter', 'weight', 'hue']]
...
test = pd.DataFrame(test, columns=[
                     'height', 'diameter', 'weight', 'hue', 'material'])
y_test = train["material"]
x_test = train[['height', 'diameter', 'weight', 'hue']]
...
count=len(x_train)
def random_centrod(x_train,k):
    count=len(x_train)
    centroids=[]
    for i in range(k):
        centroid_idx=np.random.randint(0,count-1)
        centroid=x_train[centroid_idx]
        centroids.append(centroid)
    return centroids

# random_centrod(count,3)
...
def Kmeans(centroids,k):
    ...
    for idx,point in enumerate(x_train):
        for c in centroids:
            ...
            temp_dis.append(dist)
        x=np.argmin(temp_dis)
        clusters[x].append(idx)
        temp_dis=[]
    return clusters 

# clusters=Kmeans(1,3)
def calculate_new_centroids(clusters, x_train,k):
    centroids = np.zeros((k, 4)) # row , column full with zero
    for idx, cluster in enumerate(clusters):
        new_centroid = np.mean(x_train[cluster], axis=0) # find the value for new centroids
        centroids[idx] = new_centroid
    return centroids

# new_centroids=calculate_new_centroids(clusters, x_train,3)

dic={"Plastic":0,"Metal":1,"Ceramic":2}
def predict_cluster(clusters,x_train):
    ans_lis=[]
    ...
    for cluster_idx, cluster in enumerate(clusters):
            ...
            for sample_idx in cluster:
                y_pred[sample_idx] = dic[ans]
    print(ans_lis)
    return y_pred

def fit(x_train,i,k):
    centroids = random_centrod(x_train,k) # initialize random centroids
    for _ in range(i):
        clusters =  Kmeans(centroids,k) # create cluster
        previous_centroids = centroids
        centroids = calculate_new_centroids(clusters, x_train,k) # calculate new centroids
        diff = centroids - previous_centroids # calculate difference
        if not diff.any():
            break
    y_pred = predict_cluster(clusters, x_train) # predict function 
    print("Accuracy",acc(clusters,x_train))
    return y_pred

def acc(clusters,x_train):
    ans_lis=[]
    correct=[]
    y_pred = np.zeros(len(x_train))
    for cluster_idx, cluster in enumerate(clusters):
            new=y_train[cluster]
            ans=mode(new)
            new_df=pd.DataFrame(new,columns=["material"])
            x=new_df[new_df["material"]==ans].to_numpy()
            correct.append(len(x))

    # print(ans_lis)
    return np.sum(correct)/len(x_train)



print("k=3",fit(x_train,1,3))
print("k=6",fit(x_train,1,6))
print("k=9",fit(x_train,1,9))
