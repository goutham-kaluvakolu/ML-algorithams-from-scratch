...
test = pd.DataFrame(testing_data, columns=[
    'height', 'diameter', 'weight', 'hue', 'material'])
...
def bagging_predict(models, row):
    predictions = [predict(model, row) for model in models]
...
def get_random_samples(training_data, ratio):
    temp = []
    len_train = len(training_data)
    bag_size = len_train//ratio
    for count in range(bag_size):
        idx = np.random.randint(0, len_train-1)
        temp.append(training_data[idx])
    train = pd.DataFrame(temp, columns=[
        'height', 'diameter', 'weight', 'hue', 'material'])
    y_train = train["material"].to_numpy()
    x_train = train[['height', 'diameter', 'weight', 'hue']].to_numpy()
...
def bagging(train, n):
    for i in range(n):
        ...
        # gen sample data
        x_train, y_train = get_random_samples(train, ratio)
    ...
    # apply prediction on each test point with n models and pick the best class
    predictions = [bagging_predict(models, row) for row in x_test]
...
all_accuracy.append(bagging(training_data, 10))
all_accuracy.append(bagging(training_data, 50))
all_accuracy.append(bagging(training_data, 100))
print("Accuracy of model counts 1,10,50,100")
print(all_accuracy)
